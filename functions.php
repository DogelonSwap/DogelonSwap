...
if ( !function_exists('hoverex_theme_fonts_links') ) {
	function hoverex_theme_fonts_links() {
		...
		*/
		$google_fonts_enabled = ( 'off' !== esc_html_x( 'on', 'Google fonts: on or off', 'hoverex' ) );
		$custom_fonts_enabled = ( 'off' !== esc_html_x( 'on', 'Custom fonts (included in the theme): on or off', 'hoverex' ) );
...
if ( ! function_exists( 'hoverex_comment_form_agree' ) ) {
    ...
    function hoverex_comment_form_agree( $comment_fields ) {
	    ...
	    if ( ! empty( $privacy_text )
	         && ( ! function_exists( 'hoverex_exists_wp_gdpr_compliance' ) || ! hoverex_exists_wp_gdpr_compliance() )
	    ) {
